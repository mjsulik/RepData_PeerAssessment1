...
```
Excluding days with no data, the mean is `r round(mean.exclude,0)` and the median is `r round(median.exclude,0)`. 
Including days with no data, the mean is `r round(mean.noexclude,0)` and the median is `r round(median.noexclude,0)`.
...
```
The interval with the largest number of steps is `r largest.interval`. 
...
```
The mean number of steps taken each day is `r round(mean.total,0)` and the median number of steps taken each day is `r round(median.total,0)`. 
